...
from functions.network_data2 import NetworkData # a nefesi class with network data and metadata

# defining configs for later steps
SAVE_PATH = "/Users/charlotteimbert/Documents/nefesi_outputs/VGG16_subset_contrast_100"
OBJ_FINAL = os.path.join(SAVE_PATH, "Normal_class.obj")    #output path to save the network data
NF_DIR    = os.path.join(SAVE_PATH, "nf_grids")            #output path to save grids of nfs in png format
CSV_OUT   = os.path.join(SAVE_PATH, "metrics_all.csv")     #output path to save csi in csv format 
...
def main():
    ...
    # loop through each layer to compute neuron features for each layer
    for layer in LAYERS:
        n_neurons = nd.get_len_neurons_of_layer(layer)
        cols      = int(math.ceil(math.sqrt(n_neurons)))
        rows      = int(math.ceil(n_neurons / cols))

        #1) Make the NF grid based on normalized activations
        ...
        for idx in range(n_neurons):
            ...
            all_records.append({
                "layer": layer,
                "neuron_idx": idx,
                "max_activation": max_act, #the max activation value of the neuron across all images
                "csi":            csi_val #computed csi of the neuron
            })

    # 3) Write out CSV
    df = pd.DataFrame(all_records)
    df.to_csv(CSV_OUT, index=False)
    print(f"Wrote metrics CSV → {CSV_OUT} ({len(df)} rows)") #logging to keep track
