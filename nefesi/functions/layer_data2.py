...
def gradient_rf(model, input_shape, output_layer, main_input=None):
    ...
    # Put model into train mode so we can zero‐out weights
    model = model.train()
    ...
    # Infer device from the model parameters (CPU or GPU)
    device = next(model.parameters()).device

    # Create the input tensors on the correct device
    if main_input is not None:
        inputs = [
            torch.ones(shape, device=device, requires_grad=True)
    ...
    grad  = torch.zeros(out_activations.shape, requires_grad=True, device=device)
    grad2 = torch.zeros_like(grad)
    grad3 = torch.zeros_like(grad)

    with torch.no_grad():
        center_h = grad.shape[2] // 2
        center_w = grad.shape[3] // 2

        grad [0, 0, center_h,      center_w     ] =  1
        grad2[0, 0, center_h,      center_w     ] = -1
        grad2[0, 0, center_h,      center_w + 1 ] =  1
        grad3[0, 0, center_h,      center_w + 1 ] = -1
        grad3[0, 0, 0,             0            ] =  1

    # Compute Kernel size
    out_activations.backward(gradient=grad, retain_graph=True)
    grad_input = in_activation.grad[0,0].abs().cpu().numpy()
    grad_input /= np.max(grad_input)
    Kernel = np.max(np.where(grad_input > 1e-5)) - np.min(np.where(grad_input > 1e-5)) + 1

    # Compute Stride
    out_activations.backward(gradient=grad2, retain_graph=True)
    grad_input2 = in_activation.grad[0,0].abs().cpu().numpy()
    grad_input2 /= np.max(grad_input2)
    first_active = np.argmax(grad_input > 1e-5)
    first_active2= np.argmax(grad_input2 > 1e-5)
    Stride = first_active2 - first_active

    # Compute Padding
    out_activations.backward(gradient=grad3, retain_graph=True)
    grad_input3 = in_activation.grad[0,0].abs().cpu().numpy()
    grad_input3 /= np.max(grad_input3)
    Padding = Kernel - np.argmin(grad_input3 > 1e-5)
