...
def _load_single_image(
    ...
    """Loads an image, searching src_dataset for its basename if necessary."""
    grayscale = (color_mode == 'grayscale')

    # Try 1: direct path
    direct = os.path.join(src_dataset, img_name)
    if os.path.exists(direct):
        path = direct
    else:
        # Try 2: recursive search for just the filename
        basename = os.path.basename(img_name)
        pattern  = os.path.join(src_dataset, '**', basename)
        matches  = glob.glob(pattern, recursive=True)
        if matches:
            path = matches[0]
        else:
            raise FileNotFoundError(
                f"Could not find {img_name!r} (basename={basename!r}) under {src_dataset!r}"
            )
