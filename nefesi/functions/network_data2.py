...
class NetworkData(object):
    ...
    def eval_network(self, layer_names = None,
        ...
        for n_batches, imgs in enumerate(data_batch.iterator):
            images = imgs[0]
            actual_file_names = [os.path.relpath(x, start=self.dataset.src_dataset)for x in imgs[2]]
    ...
    def most_decreased_index(self, pre_indexes, post_indexes, print_indexes_decreasing = False):
        ...
        index_decreasing_by_key = np.array(list([(key, np.sum(indexes_decreased[key]['value'])) for key in indexes_decreased]),
        ...
        max_type_by_index = np.array(list([indexes_decreased[key][np.argmax(indexes_decreased[key]['value'])] for key in indexes_decreased]))
    ...
    def indexes_decreasing_matrix_as_string(self, indexes_decreased):
        string = ''
        for concept, decreasing in list(indexes_decreased.items()):
    ...
    def indexes_decreasing(self, pre_indexes, post_indexes):
        indexes_decreased = {}
        for key in list(post_indexes.keys()):
    ...
    def get_selective_neurons(self, layers_or_neurons, idx1, idx2=None,
        ...
        if type(layers_or_neurons) is list or type(layers_or_neurons) is str:
            ...
            sel_idx = list(layers_or_neurons.keys())[0]
            values = list(layers_or_neurons.values())[0]
    ...
    def load_from_disk(file_name, model_file=None):
        ...
        if not os.path.isdir(my_net.dataset.src_dataset):
            folder = eval(input("Enter dataset folder:"))
            while not os.path.isdir(folder):
                folder = eval(input("Not exists. Enter dataset folder:"))
            ...
            if os.path.isdir(os.path.join(base_folder[0], base_folder[1]+"Segmented")):
                ...
                folder = eval(input("Enter segmented dataset folder:"))
                while not os.path.isdir(folder):
                    folder = eval(input("Not exists. Enter segmented dataset folder:"))
