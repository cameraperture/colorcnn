...
class NeuronData(object):
    ...
    def __init__(self, max_activations, batch_size, buffered_iterations = 20):
        ...
        self.activations = np.ones(shape=self._buffer_size)*-10000
    ...
    def sortResults(self, reduce_data = False):
        idx = np.argpartition(-(self.activations[:self._index]), list(range(self._max_activations)))[:self._max_activations]
    ...
    def max_concept_selectivity_idx(self):
        ...
        for i in range(9):
            top_concept = 'None'
            max = 0
            for key in list(self.selectivity_idx.keys()):
                if key != 'ivet_color':
    ...
    def get_keys_of_indexes(self):
        return list(self.selectivity_idx.keys())
